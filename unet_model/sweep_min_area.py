...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sweep min_area while holding the other U-Net post-processing settings fixed.")
    parser.add_argument("--repo-root", type=Path, default=None)
    parser.add_argument("--data-root", type=Path, default=None)
    parser.add_argument("--checkpoint", type=Path, default=Path("outputs/unet_model/best_unet.pt"))
    parser.add_argument("--output-csv", type=Path, default=Path("outputs/unet_model/min_area_sweep.csv"))
    parser.add_argument("--batch-size", type=int, default=4)
    parser.add_argument("--valid-size", type=float, default=None)
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--base-channels", type=int, default=None)
    parser.add_argument("--threshold", type=float, default=0.40)
    parser.add_argument("--min-areas", type=int, nargs="+", default=[1, 3, 5, 10, 20, 40, 80, 120, 160])
...
def main() -> None:
    ...
    for row in rows[:10]:
        print(
            f"min_area={row['min_area']} threshold={row['threshold']:.2f} "
