...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sweep close_kernel_size while holding the other U-Net post-processing settings fixed.")
    parser.add_argument("--repo-root", type=Path, default=None)
    parser.add_argument("--data-root", type=Path, default=None)
    parser.add_argument("--checkpoint", type=Path, default=Path("outputs/unet_model/best_unet.pt"))
    parser.add_argument("--output-csv", type=Path, default=Path("outputs/unet_model/close_kernel_size_sweep.csv"))
...
def main() -> None:
    ...
    for row in rows[:10]:
        print(
            f"close={row['close_kernel_size']} threshold={row['threshold']:.2f} "
