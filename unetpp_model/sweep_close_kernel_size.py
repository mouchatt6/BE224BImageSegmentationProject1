...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sweep U-Net++ close_kernel_size while holding other post-processing settings fixed.")
    parser.add_argument("--repo-root", type=Path, default=None)
    parser.add_argument("--data-root", type=Path, default=None)
    parser.add_argument("--checkpoint", type=Path, default=Path("outputs/unetpp_model/best_unetpp.pt"))
    parser.add_argument("--output-csv", type=Path, default=Path("outputs/unetpp_model/close_kernel_size_sweep.csv"))
    ...
    parser.add_argument("--close-kernel-sizes", type=int, nargs="+", default=[0, 3, 5, 7, 9, 11])
...
def main() -> None:
    ...
    print(
        "Fixed settings: "
        f"threshold={args.threshold:.3f} min_area={args.min_area} "
    ...
    rows = []
    for close_kernel_size in tqdm(args.close_kernel_sizes, desc="close_kernel_size sweep"):
        rows.append(
            evaluate_config(
                probabilities,
                targets,
    ...
    print("\nTop close_kernel_size configs by score_alpha_050:")
    for row in rows[:10]:
        print(
            f"close={row['close_kernel_size']} threshold={row['threshold']:.3f} "
            f"min_area={row['min_area']} dilate={row['dilation_iterations']} "
